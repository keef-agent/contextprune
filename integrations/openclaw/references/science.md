...
**Default:** `nomic-ai/nomic-embed-text-v1.5` (~137MB, 2048-token context window, Apache 2.0)
**Fallback:** `all-MiniLM-L6-v2` (22MB, 256-token context, faster on CPU)

Both run entirely locally. No external API calls. Every sentence in the context is encoded into a vector. Pairwise cosine similarity is computed across all sentences. Sentences above the similarity threshold (default: 0.82) that duplicate earlier sentences are removed.

## Why Non-Redundant Contexts See 0% Reduction

Non-redundant contexts pass through unchanged because none of the chunks exceed the 0.82 cosine similarity threshold. There's nothing to remove. This is the correct behavior: ContextPrune adds latency only when there's actual redundancy to eliminate.
...
2. **Batch encoding** via the embedding model — the dominant cost
...
Demonstrated 20x context compression with <1.5% accuracy loss using token-level importance scoring (perplexity from a small LM). ContextPrune uses sentence-level semantic similarity instead, which is safer for agent contexts where removing individual tokens can break reasoning chains and cause hallucination.
